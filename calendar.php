<div class="cd-schedule loading">
	<div class="timeline">
		<ul>
			<li><span>14:00</span></li>
			<li><span>14:30</span></li>
			<li><span>15:00</span></li>
			<li><span>15:30</span></li>
			<li><span>16:00</span></li>
			<li><span>16:30</span></li>
			<li><span>17:00</span></li>
			<li><span>17:30</span></li>
			<li><span>18:00</span></li>
			<li><span>18:30</span></li>
			<li><span>19:00</span></li>
			<li><span>19:30</span></li>
			<li><span>20:00</span></li>
			<li><span>20:30</span></li>
			<li><span>21:00</span></li>
			
		</ul>
	</div> <!-- .timeline -->

	<div class="events">
		<ul>
			<li class="events-group">
				<div class="top-info"><span>Monday, July 17th</span></div>

				<ul>
					<li class="single-event" data-start="14:00" data-end="15:30" data-content="event-a" data-event="event-a">
						<a href="#0">
							<em class="event-name">Fieldtrip to IrisVR with Tyler Hopf</em>
							<em>Tyler Hopf</em>
						</a>
					</li>

					<li class="single-event" data-start="16:30" data-end="18:30" data-content="event-2" data-event="event-2">
						<a href="#0">
							<em class="event-name">Intro to VR</em>
							<em>Tyler Hopf</em>
						</a>
					</li>

					<li class="single-event" data-start="19:00" data-end="21:00" data-content="event-3" data-event="event-3">
						<a href="#0">
							<em class="event-name">Overview of AR</em>
							<em>Ziv Schneider</em>
						</a>
					</li>	
				</ul>
			</li>

			<li class="events-group">
				<div class="top-info"><span>Tuesday, July 18th</span></div>

				<ul>
					<li class="single-event" data-start="16:30" data-end="18:30"  data-content="event-4" data-event="event-4">
						<a href="#0">
							<em class="event-name">Game Engines</em>
							<em>Todd Bryant</em>
						</a>
					</li>
					<li class="single-event" data-start="19:00" data-end="21:00"  data-content="event-5" data-event="event-5">
						<a href="#0">
							<em class="event-name">VR and Scientific Research</em>
							<em>David Miller</em>
						</a>
					</li>
				</ul>
			</li>

			<li class="events-group">
				<div class="top-info"><span>Wednesday, July 19th</span></div>

				<ul>
					<li class="single-event" data-start="16:30" data-end="18:30" data-content="event-6" data-event="event-6">
						<a href="#0">
							<em class="event-name">3D Scanning for VR</em>
							<em>Gabe Barcia-Colombo</em>
						</a>
					</li>

					<li class="single-event" data-start="19:00" data-end="21:00" data-content="event-7" data-event="event-7">
						<a href="#0">
							<em class="event-name">Audio in VR</em>
							<em>TK Broderick</em>
						</a>
					</li>
				</ul>
			</li>

			<li class="events-group">
				<div class="top-info"><span>Thursday, July 20</span></div>

				<ul>
					<li class="single-event" data-start="17:00" data-end="21:00" data-content="event-8" data-event="event-8">
						<a href="#0">
							<em class="event-name">Off site visit to the motion capture studio at Jay Street/Metro Tech NYU campus</em>
							<em> Todd Bryant, Matt Romein, and Kat Sullivan</em>
						</a>
					</li>
				</ul>
			</li>

			<li class="events-group">
				<div class="top-info"><span>Friday, July 21st</span></div>

				<ul>
					<li class="single-event" data-start="17:00" data-end="21:00"  data-content="event-9" data-event="event-9">
						<a href="#0">
							<em class="event-name">Demos of VR, AR, and Motion Capture projects</em>
						</a>
					</li>
				</ul>
			</li>
		</ul>
	</div>

	<div class="event-modal">
		<header class="header">
			<div class="content">
				<span class="event-date"></span>
				<h3 class="event-name"></h3>
			</div>

			<div class="header-bg"></div>
		</header>

		<div class="body">
			<div class="event-info"></div>
			<div class="body-bg"></div>
		</div>

		<a href="#0" class="close">Close</a>
	</div>

	<div class="cover-layer"></div>
</div> <!-- .cd-schedule -->